...
//private const val CURRENT_DATABASE_VERSION = 2    // ★★★ バージョンを2に更新 ★★★
//
//@Database(
//    entities = [UserEntity::class, MonsterEntity::class],
//    version = CURRENT_DATABASE_VERSION,      // ★★★ 更新したバージョンを設定 ★★★
...
//                    // ここでエラーを投げるか、他のフォールバックを検討することもできます。
//                    // ただし、getDatabaseが呼ばれる前にマイグレーションが走ることは通常ないので、
//                    // applicationContextInternal は初期化済みのはずです。
//                }
...
//                // onCreateが呼ばれるのは本当に最初の最初だけなので、ここで投入されるデータはバージョン2の初期データ
...
//                if (db.version == currentAppDbVersion && !getMonstersPopulatedFlag(context, currentAppDbVersion)) {
//                    // この条件は、MIGRATION_1_2 でフラグがクリアされた場合や、
...
//                            // ★★★ 既存の insertAllMonsters は @Insert(onConflict = OnConflictStrategy.REPLACE)
//                            //     などになっていると仮定。もしそうでなければ、
//                            //     この呼び出しの前に monsterDao.deleteAllMonsters() を呼ぶ必要があるが、
//                            //     マイグレーションでクリアしているので、ここでは不要。
